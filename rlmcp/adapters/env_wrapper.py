"""The one line you add to a training script, whatever the backend.

Nothing in this file is manager-based, or mjlab's, or IsaacLab's. Servicing,
telemetry, records, progress clips, curricula and stopping are the same work
whichever simulator is underneath, so they are written once here; a backend
subclasses :class:`RlMcpEnvWrapper` and answers two questions --
:meth:`~RlMcpEnvWrapper.build_sim_adapter` and
:meth:`~RlMcpEnvWrapper.startup_checks`. That is what keeps ``wrap()`` the same
call, and the CLI and MCP commands the same commands, on every backend.
...
* accumulation of the environment's episode logs into rlmcp's telemetry,
