...
class LiveView:
  ...
  def configure(
    ...
    if enabled is False:
      return self.stop("asked to stop")
  ...
  def tick(self) -> None:
    ...
    Three gates, cheapest first: not running, not yet due, nobody watching.
    Only past all three does anything touch the simulator, which is what keeps
    an attached-but-unwatched view off the training loop's bill.
