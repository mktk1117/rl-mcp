...
"""The recipe's own description of itself: what `rlmcp train --recipe` reads."""
...
def _manifest(record: RunRecord, session: Path, store: Any, has_package: bool,
  ...
  ladder, the seed, the env count and how long the original trained. A recipe
  whose ladder is "in curriculum.json, load it yourself" is a document; with
  this, `rlmcp train --recipe <dir>` is the launch, and a studio can press it.
  """
  info: dict[str, Any] = {}
  ...
  if not packages:
    missing.append(
        ...
        "its task; pass --task-package to `rlmcp train --recipe`, or set "
        "$TASK_PACKAGES for launch.sh)")
  seed = info.get("seed")
  if seed is None:
  ...
  manifest = {
      ...
      "from_run": record.id,
      "headline": record.headline or record.one_line(),
      "records_root": str(getattr(store, "root", "") or ""),
      "task": info.get("task") or record.task or "",
      ...
      "package": "package" if has_package else None,
      "config": "config.json",
      "curriculum": "curriculum.json",
      "policy": f"policy/{weights['name']}" if weights else None,
      "expect": dict(expectations.get("metrics") or {}),
      "verdict": record.verdict,
  }
  return manifest, missing


def _launch(manifest: dict[str, Any]) -> str:
  """launch.sh: `rlmcp train --recipe` on this directory, nothing more.

  The manifest beside it is what carries the task, the packages, the config,
  the ladder, the seed and the length; the script exists so that "run it" is
  one command with no arguments, and so that a reader can see what it does.
  """
  lines = [
      "#!/usr/bin/env bash",
      "# Generated by `rlmcp recipe build`. `rlmcp train --recipe` reads the",
      f"# {MANIFEST} beside this file -- task, packages, config.json,",
      "# curriculum.json, seed, env count and how long the original trained --",
      "# and opens the record for the rerun as recipe-<run> unless one is",
      "# named. Any flag after the record id goes straight to rlmcp-train.",
      "#",
      "#   ./launch.sh                     # record recipe-{from_run}, next number if taken".format(
          from_run=manifest.get("from_run")),
      "#   ./launch.sh 021 --no-viser      # a record you opened yourself, plus flags",
      "set -euo pipefail",
      "",
  ...
  if not manifest.get("task_packages"):
    lines += [
        "# This run did not record which modules register its task. Name them:",
        '#   TASK_PACKAGES="shand.tasks shand.rlmcp_ext" ./launch.sh',
        'read -r -a TASK_PACKAGES <<< "${TASK_PACKAGES:-}"',
        'PACKAGE_FLAGS=("${TASK_PACKAGES[@]/#/--task-package=}")',
    ]
  else:
    lines.append("PACKAGE_FLAGS=()")
  lines += [
      "",
      'rlmcp train --recipe "$HERE" ${RECORD:+--record-run "$RECORD"} \\',
      '    ${PACKAGE_FLAGS[@]+"${PACKAGE_FLAGS[@]}"} "$@"',
  ]
  return "\n".join(lines) + "\n"


def load_manifest(recipe_dir: Path | str) -> dict[str, Any]:
  """The manifest of a recipe directory, or a ValueError that says what is wrong."""
  directory = Path(recipe_dir).expanduser()
  path = directory / MANIFEST
  try:
    manifest = json.loads(path.read_text())
  except (OSError, ValueError) as exc:
    raise ValueError(
        f"No readable {MANIFEST} in {directory} ({exc}). Point --recipe at a "
        "directory written by `rlmcp recipe build`.") from exc
  if not isinstance(manifest, dict) or not manifest.get("task"):
    raise ValueError(f"{path} names no task.")
  manifest["dir"] = str(directory.resolve())
  return manifest


def open_reproduction_record(store: Any, manifest: dict[str, Any],
                             recipe_dir: Path | str) -> RunRecord:
  """The record a rerun of this recipe fills, opened before launch.

  The recipe *is* the plan -- expect.json is the prediction and the band is
  the falsifier -- so opening the record here is not skipping the discipline,
  it is the discipline applied to a run whose plan was written by a run. The
  id says what it is: ``recipe-010``, then ``recipe-010-2``, never a number
  that reads like a new experiment.
  """
  from rlmcp.records.record import Falsifier

  source = str(manifest.get("from_run") or "")
  expect = manifest.get("expect") or {}
  claimed = ", ".join(f"{k} = {v}" for k, v in expect.items()) or "no claimed metrics"
  parent = source if source and store.get_record(source) is not None else None
  record = store.new_record(
      f"recipe_{source}" if source else "recipe",
      record_id=store.free_id(f"recipe-{source}" if source else "recipe"),
      task=str(manifest.get("task") or ""),
      headline=f"Rerun of {source} from its recipe: {manifest.get('headline', '')}".strip(),
      hypothesis=(
          f"The recipe built from run {source} -- its config, its ladder, its "
          "seed and its packages -- reproduces that run's result when trained "
          "from scratch."),
      prediction=(
          f"At iteration {manifest.get('iterations') or '?'} the run lands "
          f"within the band of what {source} claimed: {claimed}."),
      falsifier=Falsifier(prose=(
          "`rlmcp recipe verify` reports a claimed metric outside the band, or "
          "the ladder does not climb the rungs the original did.")),
      change=[f"nothing by design: launched with `rlmcp train --recipe {recipe_dir}`"],
      parent=parent,
      proposed_by="recipe",
  )
