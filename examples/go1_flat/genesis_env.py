"""A Genesis quadruped locomotion environment, with the robot in the config.

Derived from Genesis's ``examples/locomotion/go2_env.py`` (Genesis, Apache
License 2.0 -- https://github.com/Genesis-Embodied-AI/Genesis). rl-mcp is
Apache 2.0 too, so this is carried here rather than vendored by reference,
which keeps the example runnable without a Genesis checkout on the path.

**One change from upstream**, marked in place: Go2Env hardcodes its URDF, while
everything else a robot needs -- joint names, default angles, PD gains, spawn
height -- already comes from ``env_cfg``. Lifting the asset out is what lets the
same environment run a Go2 or a Go1, and Genesis reads MJCF as readily as URDF,
so the Go1 here uses mjlab's own ``go1.xml``.

Nothing else is altered. Any behaviour you see is Genesis's.
"""
...
class QuadrupedEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        # The one robot-specific thing Go2Env hardcodes. Everything else the
        # robot needs -- joint names, default angles, PD gains, spawn height --
        # already comes from env_cfg, so lifting the asset out is what turns
        # this into an environment for any quadruped of this shape. Genesis
        # loads MJCF as readily as URDF, which is how a Go1 here can use the
        # very same go1.xml mjlab drives its own Go1 task from.
        robot_file = self.env_cfg.get("robot_file", "urdf/go2/urdf/go2.urdf")
        morph = gs.morphs.MJCF if str(robot_file).endswith(".xml") else gs.morphs.URDF
